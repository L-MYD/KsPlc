...
{
    public class Plc7Controller : PlcControllerBase
    {
        // PLC7配置 - 读取DB5和DB6
        ...
        private const int STATUS_LENGTH = 90;    // DB6长度为138字节

        public Plc7Controller(string ipAddress, short rack = 0, short slot = 1)
            : base("PLC7", ipAddress, rack, slot)
        {
        }
        ...
        private object ParseAllData()
        {
            ...
            {
                LogService.AddSystemLog($"解析PLC7数据异常", "数据解析",
        ...
        private void ProcessDB6BusinessLogic(object parsedData)
        {
            ...
            if (data != null)
            {
                try
                {
                    ...
                    // 处理每个站点的状态
                    //LogService.AddSystemLog($"PLC7 DB6站点状态", "状态监控",
                    ...
                    if (SationCode.Equals("1102"))
                    {
                        ...
                        {//代表此站点有货
                            ...
                            if (la2.status != "occupied" && trayNumber != "********")
                            {
                                ...
                                PLCMessageLog mes = new PLCMessageLog();
                                mes.plcip = this.IpAddress;
                                mes.direction = "Receive(获取到站点1102状态变为有货)";
                                mes.messagecontent = JsonConvert.SerializeObject(data);
                                mes.messagetimestamp = DateTime.Now.ToString("yyyy:MM:dd HH:mm:ss");
                                PLClogMapper.InsertMessageLog(mes);
                            }
                        ...
                        //代表此站点没货，没货就去清掉这里，不记录
                        {
                            ...
                            if (la2.status != "available")
                            {
                                ...
                                PLCMessageLog mes = new PLCMessageLog();
                                mes.plcip = this.IpAddress;
                                mes.direction = "Receive(获取到站点1102状态变从有货变为无货)";
                                mes.messagecontent = JsonConvert.SerializeObject(data);
                                mes.messagetimestamp = DateTime.Now.ToString("yyyy:MM:dd HH:mm:ss");
                                PLClogMapper.InsertMessageLog(mes);
                            }

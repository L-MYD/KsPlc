...
{
    public class Plc3Controller : PlcControllerBase
    {
        // PLC3配置 - 读取DB5和DB6
        ...
        public Plc3Controller(string ipAddress, short rack = 0, short slot = 1)
            : base("PLC3", ipAddress, rack, slot)
        {
        }
        ...
        private object ParseAllData()
        {
            ...
            {
                LogService.AddSystemLog($"解析PLC3数据异常", "数据解析",
        ...
        private void ProcessDB6BusinessLogic(object parsedData)
        {
            ...
            if (data != null)
            {
                try
                {
                    ...
                    // 处理每个站点的状态
                    //LogService.AddSystemLog($"PLC3 DB6站点状态", "状态监控",
                    ...
                    if (SationCode.Equals("1301"))
                    {
                        if (SationStatus.Equals("10")) // 站点为空
                        {
                            ...
                            if (la.status != "available")
                            {
                                ...
                                if (!taskTypes.Any())
                                {
                                    System.Diagnostics.Trace.WriteLine($"点位 MJ_T1 的 lanenumber 为空，无法检查任务，不清除点位");
                                    return;  // 退出 CheckStationStatus 方法，或只退出当前站点的处理
                                }
                                // 原子操作：如果没有进行中的任务，则清除点位（状态设为 available，容器号清空）
                                int affected = LocationInfoMapper.ClearIfNoTasks("MJ_T1", taskTypes);
                                if (affected > 0)
                                {
                                    // 清除成功，执行解绑
                                    UnBind unBind = new UnBind();
        ...
        private bool WriteStringToPLC(int dbBlock, int startAddress, string value, int maxLength)
        {
            ...
            {
                LogService.AddSystemLog($"PLC3写入字符串到PLC失败", "PLC通信",
                    $"异常: {ex.Message}, DB{dbBlock}.DBB{startAddress}, 值: {value}", "ERROR", "PLC3");
                return false;
            }
